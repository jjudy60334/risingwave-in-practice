...
| **StarRocks** | ⏳ 要 `REFRESH` 之後才變 | Async MV 靠 refresh，不是事件驅動 |
...
docker compose up -d
# 等容器 ready（見各資料夾 README 註解），再跑 demo.sql（指令見下）
...
- **StarRocks**（先等 CN 註冊 `SHOW COMPUTE NODES` Alive: true；**mysql client 會被 `--` 註解卡住，先 strip 掉**）：
...
- image tag：ClickHouse `24.8`、RisingWave `latest`（官方版本 tag 帶 hash 後綴，故用 latest）、StarRocks `fe-ubuntu`/`cn-ubuntu`/`minio` `3.3-latest`/`latest`。
